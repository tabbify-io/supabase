...
/**
 * What is driving Storage Size — live objects versus data retained by versioning
 * — attributed to the buckets responsible. Rendered via `additionalInfo` so the
...
export const StorageRetentionBreakdown = () => {
  ...
  const retainedBytes = totals.noncurrentVersions + totals.softDeleted
  const bucketsWithRetainedData = byBucket.filter(
    ...
    <div className="space-y-4">
      <div>
        ...
        <div className="flex items-center justify-between py-1">
          ...
          <p className="text-xs">{formatBytes(totals.live + retainedBytes)}</p>
        </div>
      </div>
      ...
      {retainedBytes > 0 && (
        <Admonition
          type="warning"
          title={`${formatBytes(retainedBytes)} is retained recovery data`}
        >
          <p className="text-xs">
            Noncurrent versions and soft-deleted files stay billable after an overwrite or delete.
            They are freed when a lifecycle policy expires them, when the retention limit is
            reached, or when you delete the version yourself.
          </p>
        </Admonition>
      )}

      {bucketsWithRetainedData.length > 0 && (
        <div>
          <p className="mb-2 text-sm">Retained data by bucket</p>
          {bucketsWithRetainedData.map((bucket) => (
            <div key={bucket.bucket} className="flex items-center justify-between border-b py-1">
              ...
              <div className="flex items-center gap-x-3">
                ...
                <p className="w-16 text-right text-xs">
